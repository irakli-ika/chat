...
$message_id = insertFieldsGetId($db, 'messages', "`message`,`sender_id`, `conversation_id`", "'{$message}', '{$sender_id}', '{$conversation_id}'");

foreach($users_id as $user_id) {    
    insertFields($db, 'received_messages', "`message_id`,`receiver_id`, `conversation_id`", "'{$message_id}', '{$user_id['user_id']}', '{$conversation_id}'");
}